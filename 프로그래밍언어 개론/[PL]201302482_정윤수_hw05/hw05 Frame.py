...
class Node (object):
    ...
    def __str__(self):
        ...
        if self.value is not None and self.type == TokenType.LIST and self.value.type == TokenType.QUOTE:
            result =  '\'' + str(self.value.next)
        elif self.type == TokenType.LIST and self.value is None:
            result = "( )"
...
class BasicPaser(object):
    ...
    def _create_node(self, token):
        ...
        if token is None:
            ...
            return Node(TokenType.CAR)
        elif token.type is CuteType.CONS:
            return Node(TokenType.CONS)
